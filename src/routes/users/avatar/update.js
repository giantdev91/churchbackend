...
async function updateUserAvatarRH(req, res) {
	const file = await sharp(req.file.path).resize(200).toBuffer();

	const response = await UploadAvatar(file, 'avatars', req.user.id); // buffer, type, and user id
...
export default updateUserAvatarRH;
